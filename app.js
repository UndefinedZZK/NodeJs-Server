...
var dashboardGetQueryJson2 = "http://localhost:9000/getQueryJson2";
var dashboardnrOfSwipes = "http://localhost:9000/getNrOfSwipes";
var dashboardUserRank = "http://localhost:9000/getUserRank";
...
function mongoAnalytics(userID, gymMonth) {
	...
	MongoClient.connect(url, function (err, db) {
	  if (err) {
		...
		monthlyStatisticsLeast(gymMonth);
			...
			function(err,result) {
				 ...
				 if(err){
                      ...
                      if(result && result[0]){
						  ...
						  nrSwipes = result[0].count;
						  var json = {"numberOfSwipes": result[0].count};
						  sendToDashboard(dashboardnrOfSwipes, json);
					  }
...
function findUserRank(gymMonth,user_swipes_month) {
	MongoClient.connect(url, function (err, db) {
		if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
		} else {
				 ...
				 if(err){
                      ...
                      if(result && result[0]){ 
						console.log("User's Rank: ", result[0].count); 
						 var json = {"userRank": result[0].count};
						sendToDashboard(dashboardUserRank,json);}
			});
		}
	});
}

function monthlyStatisticsLeast(gymMonth){
	MongoClient.connect(url, function (err, db) {
		if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
		} else {
			...
			function(err,result) {
				 if(err){
                      ...
                      if(result && result[0]){ console.log("Statistics: ", result); var json = result; sendToDashboard(dashboardGetQueryJson,json);}
			});
		}
	});
}

function monthlyStatisticsTop(gymMonth){
	MongoClient.connect(url, function (err, db) {
		if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
		} else {
			...
			function(err,result) {
				 if(err){
                      ...
                      if(result && result[0]){ console.log("Statistics: ", result); var json = result; sendToDashboard(dashboardGetQueryJson2,json);}
			});
		}
	});
}
function getNeighbouringValues(gymMonth,user_swipes_month) {
		...
		if (err) {
			...
			function(err,result) {
				 if(err){
                      ...
                      if(result && result[0]){ console.log(result); json1 = result;}
			});
			...
			function(err,result) {
				 if(err){
                      ...
                      if(result && result[0]){ console.log(result); json2 = result;}
			});
			if (json1 != "" || json2 != "")
			{
				json = ""+json1+""+json2+"";
...
function sendToDashboard(requestUrl, requestData) {		
		console.log("From dashboard", requestData);
		request({
		  uri: requestUrl,
		  method: "POST",
		  json: true,
	...
	if(response.statusCode == 200) { 
		MongoClient.connect(url, function (err, db) {
		  if (err) {
			...
			console.log('Connection established to', url);
			getNrOfSwipes(request.body.MemberID_Hash,request.body.Date_Key_Month);
		  }
		});
	  response.send("success");
    }else{
...
app.post('/findUserRank', function(request, response) {
	if(response.statusCode == 200) { 
		MongoClient.connect(url, function (err, db) {
		  if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
			...
			console.log('Connection established to', url);
			if(nrSwipes!=""){findUserRank(request.body.Date_Key_Month,nrSwipes)};
		  }
		});
	  response.send("success");
    }else{
...
app.post('/monthlyStatisticsLeast', function(request, response) {
	if(response.statusCode == 200) { 
		MongoClient.connect(url, function (err, db) {
		  if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
			...
			console.log('Connection established to', url);
			monthlyStatisticsLeast(request.body.Date_Key_Month);
		  }
		});
		response.send("success");
    }else{
...
app.post('/monthlyStatisticsTop', function(request, response) {
	if(response.statusCode == 200) { 
		MongoClient.connect(url, function (err, db) {
		  if (err) {
			console.log('Unable to connect to the mongoDB server. Error:', err);
			...
			console.log('Connection established to', url);
			monthlyStatisticsTop(request.body.Date_Key_Month);
		  }
		});
		response.send("success");
    }else{
	...
	if(response.statusCode == 200) { 
		MongoClient.connect(url, function (err, db) {
			if (err) {
				...
				console.log('Connection established to', url);
				if(nrSwipes!=""){getNeighbouringValues(request.body.Date_Key_Month,nrSwipes);}
			}
		});
	response.send("success");
    }else{
